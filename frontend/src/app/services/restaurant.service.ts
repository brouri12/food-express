...
export class RestaurantService {
  ...
  getById(id: number): Observable<Restaurant & { averageRating?: number }> {
    return forkJoin({
      restaurant: this.http.get<Restaurant>(`${this.base}/${id}`),
      average: this.ratingService.getAverageByRestaurant(id)
    }).pipe(
      map(({ restaurant, average }) => ({
        ...restaurant,
