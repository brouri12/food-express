      ...
      <aside [class]="'flex flex-col bg-gray-900 text-white transition-all duration-300 ' + (collapsed ? 'w-16' : 'w-64')">
        ...
        <div *ngIf="!collapsed" class="px-4 py-3 border-b border-gray-700">
          <div class="bg-orange-500/20 border border-orange-500/30 rounded-lg px-3 py-2">
            ...
            <p class="text-white text-sm font-medium">{{ auth.currentUser()?.firstName }}</p>
          </div>
        </div>
        ...
        <div class="p-3 border-t border-gray-700">
          ...
          <button (click)="auth.logout()" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-red-400 hover:bg-red-900/30 transition-colors">
            <span class="text-xl">🚪</span>
            <span *ngIf="!collapsed" class="font-medium text-sm">Déconnexion</span>
          </button>
        </div>
      </aside>
...
export class AdminLayoutComponent {
  ...
  navItems = [
    ...
    { path: '/admin/menus', icon: '🍽️', label: 'Menus & Plats' },
